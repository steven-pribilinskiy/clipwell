---
title: "ADR-0004: Pre-warmed picker window + global hotkey"
...
- Measured (via `bench/run-bench.ps1`): cold first show ~170ms (one-time window
  realization); a warm **hidden→shown** cycle is **~16ms** (≈ one 60Hz frame). The
  synchronous render is ~1ms — the remainder is the OS show/activate/focus. This is
  on par with the original WPF app and well below the threshold of perceptible lag.
