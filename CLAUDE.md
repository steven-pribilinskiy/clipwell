...
2. **Refresh screenshots for the affected states.** Media lives at
   ...
   - **Windows native** (`windows-*`) is captured by `bench/capture-shots.ps1` (PrintWindow;
     windowless `.exe`, off-screen, no focus steal). Also runs in the **Screenshots**
     workflow (`.github/workflows/screenshots.yml`, manual) on a Windows runner.
   - **Web** baselines + media are regenerated by the **Screenshots** workflow's web job
     (`bun run test:visual:update`), which also seeds the Linux baselines the gate needs.
   - **macOS / Linux native** (`macos-*` / `linux-*`) are captured by `bench/capture-native.ps1`
     (pwsh; grabs the real window with the OS tool — `import` under Xvfb on Linux,
     `screencapture` on macOS — since PrintWindow is Windows-only and RenderTargetBitmap
     cropped the content). Wired into the **Screenshots** workflow's `linux-native` /
     `macos-native` jobs. **Validate in CI**, then add `macos`/`linux` to `DEFAULT_SURFACES`
     in `docs/src/components/shot-switcher.tsx` (the switcher only shows committed surfaces,
     so they're additive).
