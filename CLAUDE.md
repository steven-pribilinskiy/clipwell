...
2. Add or refresh **media** (see the **Parity** rule for the per-surface details): a
   screenshot (and a short usage clip where it helps) of the new behavior. Per-state
   picker screenshots live under `docs/public/media/shots/<state>/<surface>-<theme>.png`
   and embed via `<ShotSwitcher>`; single-surface media (e.g. the usage clip) stays flat
   under `docs/public/media/` via `<ThemedShot>`/`<ThemedClip>`. Re-capture whenever the
...
## Parity — HARD RULE (both UIs, always)
There are **two** pickers of the same product: the native `ui/` (Avalonia) and the web
`webui/` (Solid). They must never drift in features or behavior.
1. **Every user-facing feature or behavior change lands in BOTH** `ui/` and `webui/` in
   the same change — a new filter, action, setting, shortcut, or layout tweak is not
   "done" until it exists in both. If a capability is genuinely platform-specific (e.g.
   .NET-only action plugins load only in Avalonia), say so explicitly in the PR.
2. **Refresh screenshots for the affected states.** Media lives at
   `docs/public/media/shots/<state>/<surface>-<theme>.png`
   (`surface` = `windows` | `macos` | `linux` | `web`; `state` = picker, detail,
   quicklook, actions, grouped, settings, diagnostics). Embed via the `<ShotSwitcher>`
   component so readers flip between surfaces in the docs.
   - **Web** (`web-*`) is captured locally + in CI by the Playwright suite
     (`cd webui && npx playwright test` — Node, not the Bun runner). It is also the
     **automated visual-regression gate**: an unintended web pixel change fails CI; an
     intended one is re-baselined with `npx playwright test --update-snapshots`.
   The **Screenshots** workflow (`.github/workflows/screenshots.yml`, manual dispatch)
   regenerates everything and uploads **artifacts** — download with `gh run download <id> -n
   <artifact>` and commit (the auto-PR path was dropped; it hit a runner credential bug).
   - **Windows native** (`windows-*`): `bench/capture-shots.ps1` (PrintWindow; windowless
     `.exe`, off-screen, no focus steal). Locally or the workflow's `windows-native` job.
   - **Linux native** (`linux-*`): `bench/capture-native.ps1 linux` — the Avalonia picker
     under Xvfb, grabbed window-by-window with ImageMagick `import` (PrintWindow is
     Windows-only; RenderTargetBitmap cropped the content, so OS-tool grab it is). Runs in
     the workflow's `linux-native` job (Xvfb + openbox). **In `DEFAULT_SURFACES`.**
   - **Web** baselines + docs media: the workflow's web job (`bun run test:visual:update`)
     produces the per-OS Linux baselines (`tests/__screenshots__/*-linux.png`) the gate
     needs, plus `shots/*/web-*.png`.
   - **macOS native** (`macos-*`): **not captured** — `screencapture` on CI hits a
     screen-recording (TCC) permission prompt headless CI can't grant (the picker renders
     fine; it just can't be grabbed). Needs a self-hosted Mac; `capture-native.ps1` keeps
     the `macos` branch. Add `macos` to `DEFAULT_SURFACES` once its media is committed.
3. **Drift detection is human, not pixel.** Native and web use different renderers/fonts,
   so **never** pixel-diff native against web. Cross-surface parity is reviewed by eye via
   the in-docs `ShotSwitcher`; the parity rule (1) is the real guardrail. The only
   automated visual-regression is **web-vs-its-own-baseline**.
